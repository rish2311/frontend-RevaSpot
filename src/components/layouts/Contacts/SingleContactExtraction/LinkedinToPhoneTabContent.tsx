...
const schema = z.object({
  ...
  emailType: z.enum(['professional_email', 'personal_email', 'phone']),
})
...
const SingleForm = ({
  ...
  onSuccess: (response: LeadContactTrackingResult, contactType: string) => void
  onError: () => void
  onReset?: () => void
}) => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: (response: LeadContactTrackingResult) => {
      const contactType = form.getValues('emailType')
      onSuccess(response, contactType)
    ...
    <Form {...form}>
      ...
      >
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              <FormLabel className='font-medium text-secondary-foreground'>Contact Type</FormLabel>
              <FormControl>
                <RadioGroup
                ...
                >
                  <FormItem className='flex items-center space-x-2'>
                    ...
                    <FormLabel className='font-normal'>Professional Email</FormLabel>
                  </FormItem>
                  <FormItem className='flex items-center space-x-2'>
                    <FormControl>
                      <RadioGroupItem value='personal_email' />
                    </FormControl>
                    <FormLabel className='font-normal'>Personal Email</FormLabel>
                  </FormItem>
                  <FormItem className='flex items-center space-x-2'>
                    <FormControl>
...
const LinkedinToContactTabContent = () => {
  const [leadTrackingInfo, setLeadTrackingInfo] = useState<LeadContactTrackingResult | null>(null)
  const [contactType, setContactType] = useState<string>('professional_email')
  ...
  const { data: leadData, isFetching, isError } = useQuery({
    ...
    refetchInterval: (query): number | false => {
      ...
      if (status === 'failed' && !leadTrackingError) {
        ...
        setIsLoading(false)
        const infoType = contactType === 'phone' ? 'phone' : 'email'
        toast.error(`Could not find ${infoType} information`)
        return false
      }
  ...
  useEffect(() => {
    ...
    if (data.status === 'completed') {
      ...
      if (contactType === 'phone') {
        // Handle phone data
        const successfulPhoneStatuses = ['Match', 'Unknown', 'Partial Match']
        
        if (data.phone_status === 'unfound' || data.phone === null) {
          setUnfoundOrFailed(true)
          toast.error('Could not find phone information')
        } else if (successfulPhoneStatuses.includes(data.phone_status)) {
          setLeadContact({ ...data })
        ...
        // Handle email data
        if (data.email_status === 'unfound' || data.email === null) {
          setUnfoundOrFailed(true)
          toast.error('Could not find email information')
        } else if (data.email_status === 'valid') {
          setLeadContact({ ...data })
          setUnfoundOrFailed(false)
          queryClient.invalidateQueries({ queryKey: ['credits'] })
          queryClient
  ...
  }, [leadData, isError, queryClient, contactType])

  useEffect(() => {
    if (leadTrackingInfo?.contact_id && isFetching) {
      setIsLoading(true)
    }
  }, [leadTrackingInfo, isFetching])

  const onHandleSuccess = (data: LeadContactTrackingResult, selectedContactType: string) => {
          ...
          {requestPending ? (
            ...
            <>
              <TabsContent value='Profile Card'>
                <ContactCard contactData={leadContact} contact_type={contactType} />
              </TabsContent>
...
export default LinkedinToContactTabContent
